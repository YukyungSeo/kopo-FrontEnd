 ...
 *      예외가 발생했을 때 처리하는 부분 
 * } finally {
 *      try, catch 안에 있는 내용이 다 샐행되고 나서도 반드시 실행되는 부분
 * }
 */
 ...
 *          //TypeError 발생 시 처리할 내용
 *      } else if(e instancefo ReferenceError) {
...
//             throw new Error(a + " is not an Array"); // 예외를 발생
//         }
//     } catch (error) {
//         console.log("배열 타입이 아닙니다." + error);
//         console.error(error);
//     } finally {
 ...
 * - async 함수는 함수안의 await 구문과 함께 비동기 작업을 해야한다.
 ...
 * - 그 동안 async 함수는 이러는 비동기 작업이 완료될 때까지 
 *      일시 중지 상태로 비동기 작업(Promise)의 해결(resolve)을 기다린다.
...
// const job = doJob("김병관", {stamina: 100});
